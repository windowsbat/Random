...
async def select_winners(contest_id: str):
    ...
    for user_id in winners_ids:
        try:
            # Используем ID канала, чтобы получить информацию о пользователе
...
async def send_welcome(message: types.Message):
    await message.reply(
        ...
        "или /cancel_contest [ID канала] для отмены розыгрыша."
    )
...
async def process_contest_input(message: types.Message):
    ...
    try:
        ...
        # 1. Получаем информацию о чате и ID
        ...
        if not bot_member.is_chat_admin() or not bot_member.can_post_messages:
            await message.reply(
                f"❌ **У бота нет необходимых прав!**\n\n"
                f"Бот должен быть администратором в канале **{channel_username}** и иметь право на **Публикацию сообщений**."
            )
            return

        # 3. Сохранение и планирование
